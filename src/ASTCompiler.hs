...
data EvaledNode
  = EvaledInt Int
  | EvaledFloat Float
  | EvaledArray [EvaledNode]
  | Undefined

eval :: Node -> State Bytecode EvaledNode
eval (IntV nb) = pure $ EvaledInt nb
eval (FloatV nb) = pure $ EvaledFloat nb
eval (ArrayV nodes) = EvaledArray <$> mapM eval nodes
eval _ = pure Undefined
